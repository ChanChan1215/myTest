...
def main():
    ...
    driver.get(url)
    #點選"打野"的按鈕
    masterClk = driver.find_element(By.CSS_SELECTOR,"a.btn-dan[data-dan='3']")
    masterClk.click()
    driver.find_element(By.CLASS_NAME, "btn-place-mid").click()
    ...
    df.info()
    #---------------------------------
    write_dir = Path("WebCrawler/LOL_KING")
    write_dir.mkdir(exist_ok=True)

    df.to_csv(
        write_dir/"LOL_KING_Mid.csv",
        header=True,
        index=False
    )
